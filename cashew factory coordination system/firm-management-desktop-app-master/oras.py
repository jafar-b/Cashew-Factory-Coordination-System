...
def insert_payment():
    ...
    try:
        ...
        sql = "insert into payment(adate,rs,remark,type) values(date('%s'),%i,'%s','oras')"%(date.get(),rs.get(),remark.get())
        cur.execute(sql)
...
def get_last_payment():
    ...
    try:
        sql = "select * from payment where type='oras'"
        print(sql)
        cur.execute(sql)
        i=7
        for result in cur:
...
def insert_raw_material():
    ...
    try:
        ...
        sql = "insert into raw_material_oras(adate,raw,type,quantity) values(date('%s'),'%s','%s',%i)"%(date.get(),raw.get(),type.get(),quantity.get())
        cur.execute(sql)

        print(sql)
        sql = "update stock_maintenance_oras set %s=%s+%i"%(type.get()+raw.get(),type.get()+raw.get(),quantity.get())
        print(sql)
        cur.execute(sql)
...
def get_last_raw():
    ...
    try:
        sql = "select * from raw_material_oras order by id desc"
        print(sql)
        cur.execute(sql)
        i=7
        for result in cur:
...
def production_insert():
    ...
    success = True
    try:
        sql = "insert into production_oras(adate,tf,fh,ts) values(date('%s'),%i,%i,%i)"%(date.get(),two_fifty.get(),five_hundred.get(),thousand.get())
        cur.execute(sql)

        print(sql)
        sql = "update stock_maintenance_oras set tf=tf+%i,fh=fh+%i,ts=ts+%i,preform250=preform250-%f,preform500=preform500-%f,preform1000=preform1000-%f,lable250=lable250-%i,lable500=lable500-%i,lable1000=lable1000-%i,caps250=caps250-%i,caps500=caps500-%i,caps1000=caps1000-%i,boxes250=boxes250-%i,boxes500=boxes500-%i,boxes1000=boxes1000-%i"%(two_fifty.get(),five_hundred.get(),thousand.get(),p250,p500,p100,l250,l500,l100,c250,c500,c100,b250,b500,b100)
        print(sql)
        cur.execute(sql)
...
def get_last_production():
    ...
    try:
        sql = "select * from production_oras order by adate desc"
        cur.execute(sql)
        i=4
        for result in cur:
...
def stock_maintain():
    ...
    sql="select * from stock_maintenance_oras"
    cur.execute(sql)
    result = cur.fetchone()
    print(result)
...
def main():
    ...
    flag='oras'
    oras=Tk()
    oras.configure(background="white")
    oras.title('oras')
    oras.state("zoomed")
    #billingsto.wm_iconbitmap('favicon.ico')
    #Label(oras,text='-'*48+'oras'+'-'*49).grid(row=0,column=0,columnspan=7,sticky='W')

    side_menu = tk.Frame(oras,background="white")
    ...
    tk.Button(side_menu,width=20,text='Back to Main Menu',font=("Belwe Bd BT",15),command=oras.destroy).grid(row=0, column=5)
    ...
    sw= ScrolledWindow(oras)
    sw.pack()

    middle_section = tk.Frame(sw.window,background="white")
    Label(middle_section,text='-'*48+'Oras'+'-'*49,font=("Belwe Bd BT",15),background="white").grid(row=0,column=0,columnspan=9,sticky='N')
